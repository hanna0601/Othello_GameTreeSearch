...
def minimax_min_node(board, color, limit, caching = 0):
    ...
    if caching != 0 and (board, get_competitor_color(color)) in cached_states.keys():

        return cached_states[(board, get_competitor_color(color))] # return the action of the min node and the corresponding utility for max node
    ...
    possible_moves = get_possible_moves(board, get_competitor_color(color))  # get the possible moves for the min player

    # if the possible moves are empty, or the limit is reduced to 0, return (NULL, utility)
    if possible_moves == []:
        return None, compute_heuristic(board, color)  # this is the utility for the max player
    ...
    for i, j in possible_moves:
        new_board = play_move(board, get_competitor_color(color), i, j)
    ...
    if caching != 0:
        cached_states[(board, get_competitor_color(color))] = (min_action, min_utility)
...
def alphabeta_min_node(board, color, alpha, beta, limit, caching = 0, ordering = 0):
    ...
    # if the current board state has been visited before, return the value from the cache
    if caching != 0 and (board, get_competitor_color(color), alpha, beta) in cached_states:

        return cached_states[(board, get_competitor_color(color), alpha, beta)]
    
    if limit == 0:
        return None, compute_heuristic(board, color)

    # get the possible moves for the min node
    possible_moves = get_possible_moves(board, get_competitor_color(color)) # returns a list of (column, row) tuples.
    ...
    if ordering != 0:
        ...
        competitor = get_competitor_color(color)

        possible_moves = sorted(possible_moves, key = lambda x: compute_heuristic(play_move(board, competitor, x[0], x[1]), color), reverse = False)
    ...
    if caching != 0:
        cached_states[(board, get_competitor_color(color), alpha, beta)] = (min_action, min_utility)
...
def alphabeta_max_node(board, color, alpha, beta, limit, caching = 0, ordering = 0):
    ...
    if ordering != 0:
        ...
        possible_moves = sorted(possible_moves, key = lambda x: compute_heuristic(play_move(board, color, x[0], x[1]), color), reverse = True)
...
def run_ai():
    ...
    """
    print("hhh") # First line is the name of this AI
